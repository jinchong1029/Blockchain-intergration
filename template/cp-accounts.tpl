         ...
         <div class="row">
            <div class="col-sm-12">
               <table class="table table-striped- table-bordered table-hover dataTable no-footer dtr-inline" id="accounts_table" role="grid" aria-describedby="fullz_table_info" style="width: 1541px;">
                  ...
                  <tbody>
                    {foreach $allAccounts as $eachAccount=>$ea}
                     <tr role="row" class="even" id="{$ea['id']}">
                        <td data-target="eaId">{$ea['id']}</td>
                        <td data-target="eaType"><span class="badge badge-primary">{$ea['type']}</span></td>
                        <td data-target="eaInfo">{$ea['info']}</td>
                        <td data-target="eaPrice">{$ea['price']}</td>
                        {if $ea['status'] eq '0'}
                        ...
                        {/if}
                        <td nowrap="" style="">
                           <button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-role='edit' data-id="{$ea['id']}" title="Edit"><i class="la la-edit" style="font-size:24px;"></i></button>
                           <button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-role='delete' data-id="{$ea['id']}" title="Delete"><i class="la la-trash" style="font-size:24px;"></i></button>
                           <button type="button" class="btn btn-sm btn-clean btn-icon btn-icon-md" data-role='recheck' data-id="{$ea['id']}" title="Recheck"><i class="la la-refresh" style="font-size:24px;"></i></button>
                        </td>
                        <td hidden data-target="eaUsername">{$ea['username']}</td>
                        <td hidden data-target="eaPassword">{$ea['password']}</td>
                     </tr>
                     {/foreach}
                  </tbody>
               </table>
